node {
   ...
   stage('Preparation') {
      ...
      checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: '${GIT_BASE_URL}root/CouponOrganizer.git']]])
      mvnHome = tool 'M3'
   }
   stage('Build') {
      // Run the maven build
